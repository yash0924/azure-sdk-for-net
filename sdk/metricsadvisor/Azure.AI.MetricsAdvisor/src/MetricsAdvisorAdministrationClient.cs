...
namespace Azure.AI.MetricsAdvisor.Administration
{
    ...
    public class MetricsAdvisorAdministrationClient
    {
        ...
        public virtual AsyncPageable<DataFeed> GetDataFeedsAsync(GetDataFeedsOptions options = default, CancellationToken cancellationToken = default)
        {
            ...
            string creator = options?.GetDataFeedsFilter?.CreatorEmail;
        ...
        public virtual AsyncPageable<NotificationHook> GetHooksAsync(GetHooksOptions options = default, CancellationToken cancellationToken = default)
        {
            async Task<Page<NotificationHook>> FirstPageFunc(int? pageSizeHint)
            {
                ...
                try
                {
                    Response<HookList> response = await _serviceRestClient.ListHooksAsync(options?.HookName, options?.Skip, options?.MaxPageSize, cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value, response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            async Task<Page<NotificationHook>> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                ...
                try
                {
                    Response<HookList> response = await _serviceRestClient.ListHooksNextPageAsync(nextLink, options?.HookName, options?.Skip, options?.MaxPageSize, cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value, response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
        ...
        public virtual Pageable<NotificationHook> GetHooks(GetHooksOptions options = default, CancellationToken cancellationToken = default)
        {
            Page<NotificationHook> FirstPageFunc(int? pageSizeHint)
            {
                ...
                try
                {
                    Response<HookList> response = _serviceRestClient.ListHooks(options?.HookName, options?.Skip, options?.MaxPageSize, cancellationToken);
                    return Page.FromValues(response.Value.Value, response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            Page<NotificationHook> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                ...
                try
                {
                    Response<HookList> response = _serviceRestClient.ListHooksNextPage(nextLink, options?.HookName, options?.Skip, options?.MaxPageSize, cancellationToken);
                    return Page.FromValues(response.Value.Value, response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
